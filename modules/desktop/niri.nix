...
let
  ...
  gamescope-json = ''
    {"title": "Steam", "body": "Launching Gamescope..."}
  '';
  gamescope-launch =
    [
      "noctalia-shell"
...
in
{
  ...
  programs.niri.settings = {
    ...
    binds = {
      ...
      "Mod+Backslash".action.spawn = [ "niri-window-details" ];

      # Peekaboo
      "Mod+P".action.spawn = [ "niri-peekaboo" ];

      # Workspace helper replacing default workspace binds
      "Mod+1".action.spawn = [ "niri-workspace-helper" "1" ];
      "Mod+2".action.spawn = [ "niri-workspace-helper" "2" ];
      "Mod+3".action.spawn = [ "niri-workspace-helper" "3" ];
      "Mod+4".action.spawn = [ "niri-workspace-helper" "4" ];
      "Mod+5".action.spawn = [ "niri-workspace-helper" "5" ];

      # show keybind-cheatsheet
      "Mod+Slash".action.spawn = noctalia "plugin:keybind-cheatsheet toggle";

      # show kde-connect
      "Mod+Period".action.spawn = noctalia "plugin:kde-connect toggle";
      ...
      "Alt+Shift+Grave".action.spawn = [ "niriswitcherctl" "show" "--works" ];

      # Session
      "Mod+Shift+E".action.quit = { };
      "Mod+Shift+P".action.spawn = noctalia "sessionMenu toggle";
      "Mod+Shift+G".action.spawn = gamescope-launch;

      # Apps
      "Mod+T".action.spawn = [ terminal ];
      "Mod+semicolon".action.spawn = [ terminal ];
      "Mod+B".action.spawn = [ browser ];
      "Mod+G".action.spawn = [ "${pkgs.google-chrome}   /bin/google-chrome" ];
